...
namespace UnityEditor.Rendering.TCdemo
{
    public class RampGenerator : EditorWindow
    {
        ...
        [MenuItem("Tools/Ramp Generator", false, 600)]
        static void OpenTool()
        {
            GetWindowTCP2();
        }

        private static RampGenerator GetWindowTCP2()
        {
            var window = GetWindow<RampGenerator>(true, "TCP2 : Ramp Generator", true);
	...
	private const string OUTPUT_FOLDER = "/Textures/Custom Ramps/";
        ...
        bool isRamp2d
        {
            get { return (editMode && editedTextureIs2d) || (!editMode && tabIndex == 1); }
        }
