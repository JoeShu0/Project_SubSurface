#ifndef CUSTOM_OCEAN_GI_INCLUDED
#define CUSTOM_OCEAN_GI_INCLUDED

#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/EntityLighting.hlsl"
#include "Packages/com.unity.render-pipelines.core/ShaderLibrary/ImageBasedLighting.hlsl"

//No shadow map needed for Ocean
/*
...
float3 SampleEnvironment(Surface surfaceWS, TRDF trdf)
{
	...
	float mip = PerceptualRoughnessToMipmapLevel(trdf.perceptualRoughness);
...
GI GetGI(float2 lightMapUV, Surface surfaceWS, TRDF trdf)
{
	GI gi;
	//GI diffuse color should becontrolled by time of the day(main directional light SUN)
	gi.diffuse = 0.0;// SampleLightProbe(surfaceWS);// SampleLightMap(lightMapUV) + SampleLightProbe(surfaceWS);
	gi.specular = SampleEnvironment(surfaceWS, trdf);

	//no static shadow on ocean surface
